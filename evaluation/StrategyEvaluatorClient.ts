...
export class StrategyEvaluatorClient {
  ...
  private buildEvaluationPrompt(request: EvaluationRequest): string {
    ...
    return [
      ...
      "",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "CRITICAL: RISK & R:R COMPUTATION (MUST DO IF DISCUSSING '1:4', 'R:R', OR 'SOUND DESIGN')",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "",
      "For the FIRST order plan in orderPlans (or the active one):",
      "- entryLow = entryZone[0], entryHigh = entryZone[1]",
      "- stop = stopPrice",
      "- target = targets[0].price (if multiple targets, compute for the one you reference)",
      "- qty = qty",
      "",
      "Compute WORST-CASE LONG (side=buy):",
      "- riskWorstPerShare = entryHigh - stop",
      "- rewardWorstPerShare = target - entryHigh",
      "- rrWorst = rewardWorstPerShare / riskWorstPerShare",
      "- dollarRiskWorst = riskWorstPerShare * qty",
      "",
      "Compute WORST-CASE SHORT (side=sell):",
      "- riskWorstPerShare = stop - entryLow",
      "- rewardWorstPerShare = entryLow - target",
      "- rrWorst = rewardWorstPerShare / riskWorstPerShare",
      "- dollarRiskWorst = riskWorstPerShare * qty",
      "",
      "Rules:",
      "- Only say 'true 1:4' if rrWorst >= 4.0.",
      "- If strategy name/description claims '1:4' but rrWorst < 4.0, explicitly call this out.",
      "- If dollarRiskWorst > maxRiskPerTrade, treat as a serious defect → SWAP or CLOSE (do not 'keep').",
      "",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "CRITICAL: ENTRY ZONE EVALUATION LOGIC (DO NOT MISLABEL 'WAITING' AS 'MISALIGNED')",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "",
      "Interpret entryZone as a desired fill band with directional confirmation semantics:",
      "- BUY entries intend to fill when price RISES into [X, Y].",
      "- SELL entries intend to fill when price FALLS into [X, Y].",
      "",
      "FOR BUY ORDERS (entryZone [X, Y]):",
      "- If current price < X: waiting for rise into zone ✅ neutral",
      "- If current price in [X, Y]: entry feasible now ✅ good",
      "- If current price > Y: can be either:",
      "  • already ran through zone earlier (possible missed), OR",
      "  • pullback/reclaim style waiting for dip below zone then rise back in ✅ neutral",
      "  Default: treat as neutral unless you have evidence it was truly missed while armed/triggered.",
      "",
      "FOR SELL ORDERS (entryZone [X, Y]):",
      "- If current price > Y: waiting for fall into zone ✅ neutral",
      "- If current price in [X, Y]: entry feasible now ✅ good",
      "- If current price < X: can be either:",
      "  • already ran through zone earlier (possible missed), OR",
      "  • pullback/reclaim style waiting for bounce above zone then fall back in ✅ neutral",
      "  Default: treat as neutral unless you have evidence it was truly missed while armed/triggered.",
      "",
      "⚠️ COMMON MISTAKES TO AVOID:",
      "- Do NOT call 'price below entry zone' a problem for BUY orders — often it's just waiting.",
      "- Do NOT call 'price above entry zone' a problem for SELL orders — often it's just waiting.",
      "",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      ...
      "",
      "Additionally for SWAP, you MUST compute rrWorst and dollarRiskWorst for the suggested strategy.",
      "",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      ...
      "- Start with FACT bullets (state, trigger/invalidations vs latest CLOSE, rrWorst, dollarRiskWorst, bars/time-in-market-hours)",
