...
import type { BarCacheService } from './BarCacheService';
...
export class BarCacheMonitor {
  ...
  constructor(
    ...
    private cacheService: BarCacheService,
  ...
  async logStats(): Promise<void> {
    try {
      ...
      const totalMemoryBytes = stats.reduce((sum, s) => sum + s.memoryBytes, 0);
      const totalMemoryMB = (totalMemoryBytes / (1024 * 1024)).toFixed(2);

      const totalHits = stats.reduce((sum, s) => sum + s.cacheHits, 0);
      const totalMisses = stats.reduce((sum, s) => sum + s.cacheMisses, 0);
      ...
      const topEntries = stats
        .sort((a, b) => b.cacheHits + b.cacheMisses - (a.cacheHits + a.cacheMisses))
        .slice(0, 5);

      logger.info('[BarCacheMonitor] Top 5 Most Active Cache Entries', {
        entries: topEntries.map((s) => ({
          symbol: s.symbol,
