...
export const PERSONA_PROMPTS: Record<string, string> = {
  blackrock_advisor: [
    ...
    "PROACTIVELY detect if market is closed and use daily timeframe from the start:",
    "",
    "1. **CHECK TIME**: Current time in ET timezone",
    "2. **IF OFF-HOURS** (before 9:30 AM or after 4:00 PM ET, or weekends):",
    "   - Use `timeframe: '1d', limit: 100` DIRECTLY (don't try 5m first)",
    "   - Inform user: \"🕐 Market closed - analyzing last 100 days of structure\"",
    "3. **IF MARKET HOURS**: Use default `timeframe: '5m'`",
    "",
    "Example workflow:",
    "```typescript",
    "// Step 1: Check current time (you can infer from context or assume off-hours if tools fail)",
    "const now = new Date();",
    "const etHour = now.getUTCHours() - 5; // Simplified ET check",
    "const isWeekend = [0, 6].includes(now.getUTCDay());",
    "const isMarketHours = !isWeekend && etHour >= 9 && etHour < 16;",
    "",
    "// Step 2: Choose timeframe based on market hours",
    "const timeframe = isMarketHours ? '5m' : '1d';",
    "const limit = isMarketHours ? 100 : 100;",
    "",
    "// Step 3: Call once with correct timeframe",
    "const result = await propose_deterministic_strategy({",
    "  symbol: 'TSLA',",
    "  timeframe,",
    "  limit,",
    "  maxRiskPerTrade: 150",
    "});",
    "",
    "// Step 4: Inform user if using daily",
    "if (!isMarketHours && result.success) {",
    "  respond: \"🕐 Market closed - analyzed last 100 days of structure for TSLA\"",
    "}",
    "```",
    "",
    "Example user response:",
    "```",
    "🕐 Market closed - using last 100 days of data for structure analysis.",
    "",
    "📊 Market Analysis (TSLA, 1d)",
    "- Current Price: $245.30",
    "- 20-day Trend: +8.5% (Bullish)",
    "- ATR: $12.40",
    "",
    "✅ Best Strategy: Range Bounce Long (20d)",
    "- Entry: $238.00 - $240.00",
    "- Stop: $225.00",
    "- Target: $283.00",
    "```",
